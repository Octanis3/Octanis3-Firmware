  ...
  NbSectorToRead = NbBlock_To_Read/32+1;
    ...
    if((((NthDataToRead+SectorStart) <= 8)&&(Tag_Density == ISO15693_LOW_DENSITY)) || (Tag_Density == ISO15693_STLEGLR_HIGH_DENSITY))
    {
      ...
      /* Over the 256th block call the Extended commands */
			if ( ISO15693_ExtendedReadMultipleBlock (Requestflags, 0x00,NthDataToRead+SectorStart,0x1F,RepBuffer ) !=ISO15693_SUCCESSCODE)
						return ISO15693_ERRORCODE_DEFAULT;	
    }      
      /*Data Temp*/
	...
	{			
        ...
        if(((Num_DataToRead <= 0xFF)&&(Tag_Density == ISO15693_LOW_DENSITY)) || (Tag_Density == ISO15693_STLEGLR_HIGH_DENSITY))
        {
         ...
         if ( ISO15693_ReadSingleBlock (Requestflags, 0x00,Num_DataToRead,RepBuffer ) !=ISO15693_SUCCESSCODE)
          ...
          if ( ISO15693_ExtendedReadSingleBlock (Requestflags, 0x00,Num_DataToRead,RepBuffer ) !=ISO15693_SUCCESSCODE)
						return ISO15693_ERRORCODE_DEFAULT;
        }					
 				 /*Data Temp*/
...
//	uint8_t WriteBuffer [8]={0x02, 0x21, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
	...
	uint16_t NthDataToWrite =0,
      ...
      {
      /* For lower blocks, or the high density tags relying on the ISO15693 ext. protocol bit */
        if ( ISO15693_WriteSingleBlock (Requestflags, 0x00,NthDataToWrite,&Data_To_Write[(NthDataToWrite - FirstBlock_To_Write)<<2],RepBuffer ) !=ISO15693_SUCCESSCODE)
          return ISO15693_ERRORCODE_DEFAULT;
      } else {
       /* Over the 256th block call the NFC-Forum Type5 Extended commands */
       if ( ISO15693_ExtendedWriteSingleBlock (Requestflags, 0x00,NthDataToWrite,&Data_To_Write[(NthDataToWrite - FirstBlock_To_Write)<<2],RepBuffer ) !=ISO15693_SUCCESSCODE)
          return ISO15693_ERRORCODE_DEFAULT;
      }
     IncBlock += 4;
		}
...
int8_t ISO15693_ExtendedLockSingleBlock ( unsigned char Flags, unsigned char *UIDin, uint16_t BlockNumber,uint8_t *pResponse)
{
	...
	DataToSend[NthByte++] = Flags;
	DataToSend[NthByte++] = ISO15693_CMDCODE_EXTLOCKBLOCK;
...
int8_t ISO15693_GetMultipleBlockSecutityStatus (unsigned char Flags, unsigned char *UIDin, unsigned char BlockNumber, unsigned char NbBlocks,uint8_t *pResponse)
{
	...
	if (PCD_IsReaderResultCodeOk (SEND_RECEIVE,pResponse) == ERRORCODE_GENERIC)
...
/**  
* @brief  	this function send an Extended Get Multiple Block Secutity Status command to contactless tag.
* @param  	Flags		:  	Request flags
* @param	UIDin		:  	pointer on contacless tag UID (optional) (depend on address flag of Request flags)
* @param	BlockNumber	:  	index of block to read
* @param	NbBlocks	: 	Number of blocks
...
int8_t ISO15693_ExtendedGetMultipleBlockSecutityStatus (unsigned char Flags, unsigned char *UIDin, uint16_t BlockNumber, uint16_t NbBlocks,uint8_t *pResponse)
{
	...
	DataToSend[NthByte++] = Flags;
	DataToSend[NthByte++] = ISO15693_CMDCODE_EXTGETSECURITYINFO;
  ...
  DataToSend[NthByte++] = NbBlocks & 0x00FF;
  DataToSend[NthByte++] = (NbBlocks & 0xFF00) >> 8;
